...
public class AppAction {
	public static App createApp(String appName, String appReturnUrl, String appNotifyUrl) {
		...
		try {
			...
			if (result != 1) {
				LogManager.mariadbLog.warn("创建app失败");
				return null;
			}
			sqlSession.commit();
		} catch (Exception e) {
			...
			LogManager.mariadbLog.error("创建app失败", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static App updateApp(String appId, String appName, boolean updateAppKey, int appState, String appReturnUrl, String appNotifyUrl) {
		...
		try {
			...
			if (result != 1) {
				LogManager.mariadbLog.warn("修改app失败");
				return null;
			}
			sqlSession.commit();
		} catch (Exception e) {
	...
	public static App getAppById(String appId) {
		...
		try {
			...
			if (app == null) {
				LogManager.mariadbLog.warn("通过appId:" + appId + "获取app为空");
			}
		} catch (Exception e) {
			if (sqlSession != null) {
				sqlSession.rollback();
			}
			LogManager.mariadbLog.error("获取app异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static List<App> getAppList(String appName, String appCreateTimeGreaterThan, String appCreateTimeLessThan, int appState, int limitStart, int pageSize) {
		...
		try {
			...
			LogManager.mariadbLog.error("查询applist失败", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static long getAppListCount(String appName, String appCreateTimeGreaterThan, String appCreateTimeLessThan, int appState) {
		...
		try {
			...
			LogManager.mariadbLog.error("查询applistcount失败", e);
			return -1;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
