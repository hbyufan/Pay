...
public class NotifyAction {
	public static Notify createNotify(String appId, String orderRecordId, int notifyType) {
		...
		try {
			...
			if (result != 1) {
				LogManager.mariadbLog.warn("创建notify失败");
				return null;
			}
			sqlSession.commit();
		} catch (Exception e) {
			...
			LogManager.mariadbLog.error("创建notify失败", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static Notify getNotify(String notifyId) {
		...
		try {
			...
			if (notify == null) {
				LogManager.mariadbLog.warn("通过notifyId:" + notifyId + "获取notify为空");
			}
		} catch (Exception e) {
			if (sqlSession != null) {
				sqlSession.rollback();
			}
			LogManager.mariadbLog.error("获取notify异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static List<Notify> getNotifyList(String appId, String orderRecordId, int notifyType, int limitStart, int pageSize) {
		...
		try {
			...
			LogManager.mariadbLog.error("获取notifylist失败", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static Notify updateNotify(String notifyId, String notifyResult) {
		...
		try {
			...
			if (result != 1) {
				LogManager.mariadbLog.warn("修改notify失败");
				return null;
			}
			sqlSession.commit();
		} catch (Exception e) {
			...
			LogManager.mariadbLog.error("修改notify异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
